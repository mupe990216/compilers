...
def ConcatenarAFNs(AFN1, AFN2, ventana, misAFNs):
    automata1 = misAFNs[int(AFN1.get())]
    automata2 = misAFNs[int(AFN2.get())]
    automataNuevo = automata1.concatenar(automata2)
...
def creacionConcatenacion(misAFNs):
    #Crear la ventana
    ventana = tk.Tk()
    ...
    #Boton
    boton = tk.Button(ventana, text="Concatenar AFNs", command=lambda: ConcatenarAFNs(listaAFN1,listaAFN2,ventana, misAFNs))
