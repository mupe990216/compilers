...
def hacerOpcional(AFN, ventana, misAFNs):
    automata = misAFNs[int(AFN.get())]
    automataNuevo = automata.opcional()
...
def creacionOpcional(misAFNs):
    ...
    #Boton
    boton = tk.Button(ventana, text="Realizar operación", command=lambda: hacerOpcional(listaAFN,ventana, misAFNs))
